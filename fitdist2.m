function [pd,gn,gl] = fitdist2(x,distname,varargin)
...
%   $Revision: 1.1.6.4 $  $Date: 2009/05/07 18:31:04 $
...
if strncmpi(distname,'kernel',max(1,length(distname)))
    distname = 'kernel';
end
...
if isempty(by)
    ...
    freq(freq==0) = NaN;
    [badin,wasnan,gidx,x,cens,freq] = statremovenan(gidx,x,cens,freq);
    if badin>0
